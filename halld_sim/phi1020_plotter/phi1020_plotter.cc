...
#include "AMPTOOLS_DATAIO/Phi1020PlotGenerator.h"
...
int main( int argc, char* argv[] ){
  ...
  if (argc < 2){
    ...
    cout << "\tphi1020_plotter <results file name> -o <output file name>" << endl << endl;
    return 0;
  }

  bool showGui = false;
  string outName = "phi1020_plot.root";
  ...
  for(auto reactionName : results.reactionList()) {
	//cout << reactionName << endl;
	plotfile->cd();
	plotfile->mkdir(reactionName.c_str())->cd();
//  string reactionName = results.reactionList()[0];
  ...
  //for (unsigned int isum = 0; isum <= sums.size(); isum++){
    ...
    for (unsigned int iplot = 0; iplot < PlotGenerator::kNumTypes; iplot++){
      ...
      for (unsigned int ivar  = 0; ivar  < TwoPiPlotGenerator::kNumHists; ivar++){
        ...
        string histname =  "";
        if (ivar == TwoPiPlotGenerator::k2PiMass)  histname += "M2K";
        ...
        else if (ivar == TwoPiPlotGenerator::kMomPiPlus)  histname += "p1mom";
        else if (ivar == TwoPiPlotGenerator::kMomPiMinus) histname += "p2mom";
        else if (ivar == TwoPiPlotGenerator::kMomProton)  histname += "pmom";
        else continue;
	...
	//  // get name of sum for naming histogram
        ...
        //plotfile->cd();
        //plotfile->cd(reactionName.c_str());
        thist->Write();
  ...
  ofstream outfile;
  outfile.open( "phi1020_fitPars.txt" );
  ...
  //// Save total intensity to .txt file
  //outfile.open("phi1020_fitIntensity.txt");
  //outfile << results.intensity().first << "," << results.intensity().second;
  //outfile.close();
  cout << results.intensity().first << results.intensity().second;
