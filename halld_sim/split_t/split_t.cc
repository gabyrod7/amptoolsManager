...
int main( int argc, char* argv[] ){
  ...
  binEdges[numBins] = highT; // initialize last bin edge
  ...
  while( ( event = in.getEvent() ) != NULL && eventCount++ < maxEvents ){
    ...
    if( ( bin < numBins ) && ( bin >= 0 ) ){
      ...
      TsumSq[bin]+=t*t;
      events[bin]++;
      if(binEdges[bin] > t )	binEdges[bin] = t;
  ...
  ofstream outfile;
  outfile.open( "tBins.txt" );
  
  for( int i = 0; i < numBins; ++i ){
    double mean = Tsum[i]/events[i];
    double RMS = sqrt(TsumSq[i]/events[i] - mean*mean);
    printf("bin %2i  %10i events  mean T %.3f  RMS %.3f lowEdge %.3f highEdge %.3f\n",
	   i,events[i],mean,RMS, binEdges[i], binEdges[i+1]);

    outfile << i << "," << mean << "," << RMS << endl;
    //outfile << i << "," << (binEdges[i] + binEdges[i+1])/2. << "," << (binEdges[i+1] - binEdges[i])/2. << endl;
    //outfile << i << "," << (binEdges[i] + binEdges[i+1])/2. << "," << binEdges[i] << "," << binEdges[i+1] << endl;
